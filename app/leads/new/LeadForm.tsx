...
export default function LeadForm() {
  ...
  // After mount (client only), pull attribution the SEO landing page stored (plus any
  // utm params on this URL) and prefill the service from the niche the visitor came
  // from — without overriding a manual choice. Post-hydration setState is intentional.
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    const attr = captureAttribution();
    if (attr.service === "junk-removal" || attr.service === "landscaping") {
      setService((prev) => prev || (attr.service as Service));
    }
  }, []);
